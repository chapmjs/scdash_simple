...
inventory_data <- data.frame(
  `Part Number` = c(
    "E2081-66555", "E2081-66556", "E2084-66501", "E2084-66508", "E2084-66522", 
  ...
  Description = c(
    "HV Driver", "Waveplate 1\"", "Motorized Flipper", "Mounting pedistal", "Holding Fork", 
  ...
  Supplier = c(
    "McDonalds", "McDonalds", "McDonalds", "McDonalds", "Wendys", 
...
server <- function(input, output) {
  ...
  output$inventoryPlot <- renderPlot({
    ...
    # Create data frame for plotting - handle NA values
    cm1 <- part$`CM #1 Inventory`
    cm2 <- part$`CM #2 Inventory`
    oem <- part$`OEM Inventory`
    
    # Create locations vector based on available data
    locations <- c("CM #1", "CM #2")
    inventory <- c(cm1, cm2)
    
    # Add OEM only if data exists
    if (!is.na(oem)) {
      locations <- c(locations, "OEM")
      inventory <- c(inventory, oem)
    }
